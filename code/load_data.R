...
metadata <- read_xlsx('data/240711_merged/metadata_merged.xlsx', col_types = 'text') %>%
  ...
  select(-c(source_material_id, feed, treatment, plate_id, organism, lat_lon, sra_number)) %>% # drop uninformative metadata
  mutate(
...
# setdiff(metadata$sampleid, colnames(table)) # all metadata sampleid's in table
